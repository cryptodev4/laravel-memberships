...
class CreatePlanSubscriptionUsageTable extends Migration
{
    ...
    public function up(): void
    {
        Schema::create(config('laravel-subscriptions.tables.plan_subscription_usage'), function (Blueprint $table) {
            ...
            $table->foreign('subscription_id')->references('id')->on(config('laravel-subscriptions.tables.plan_subscriptions'))
                  ->onDelete('cascade')->onUpdate('cascade');
            $table->foreign('feature_id')->references('id')->on(config('laravel-subscriptions.tables.plan_features'))
                  ->onDelete('cascade')->onUpdate('cascade');
        });
    }
